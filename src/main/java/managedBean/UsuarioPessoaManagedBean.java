...
public class UsuarioPessoaManagedBean {
	...
	public String salvar() {
		...
		if (daoGeneric.countPessoas() < 20) {
			...
			FacesContext.getCurrentInstance().addMessage(null,
					new FacesMessage(FacesMessage.SEVERITY_INFO, "Info:", "User saved successfully."));
		} else {
	...
	public String excluir() {
		try {
			...
			init();
			FacesContext.getCurrentInstance().addMessage(null,
					new FacesMessage(FacesMessage.SEVERITY_INFO, "Info:", "Removed successfully."));
		} catch (Exception e) {
			if (e.getCause() instanceof org.hibernate.exception.ConstraintViolationException) {
				FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "Info:",
						"There are phones registered to this user."));
			} else {
				e.printStackTrace();
			}
		}
	...
	public void addEmail() {
		if (emailUser.getEmail().isEmpty()) {
			FacesContext.getCurrentInstance().addMessage(null,
		            new FacesMessage(FacesMessage.SEVERITY_ERROR, "Error: ", "Empty Field. Enter a valid email"));
		} else {
	...
	public void upload(FileUploadEvent imageUpload) {
		String imageStr = "data:image/png;base64,"
				+ DatatypeConverter.printBase64Binary(imageUpload.getFile().getContents());
	...
	public void download() throws IOException {
		...
		UsuarioPessoa pessoa = daoGeneric.pesquisar(Long.parseLong(idForDownload), UsuarioPessoa.class);
		...
		if (pessoa.getImagem() != null) {
			...
			FacesContext.getCurrentInstance().addMessage(null,
					new FacesMessage(FacesMessage.SEVERITY_ERROR, "Alert", "User withou photo."));
		}
		init();
